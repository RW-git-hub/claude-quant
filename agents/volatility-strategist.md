...
2. **Get the causal RV forecast** (HAR-RV/EWMA/GARCH, RHS through `t−1`) — the vol-forecast skill and regime-detector agent PRODUCE this; consume their lagged forecast rather than re-deriving it. Helpers (`ewma_vol`/`garch11_fit`/`garch11_filter`/`garch11_forecast`) live in `templates/regime.py`; lag discipline in `references/time-series-regimes.md` sec 1.
