...
- `templates/options.py` — `bs_price`, `bs_delta/gamma/vega/theta/rho`, `put_call_parity_gap`, `_no_arb_bounds`, `forward_price`, and `implied_vol` (Brenner-Subrahmanyam seed → Newton → bisection on `[1e-6, 10]`, returns NaN outside bounds). Reuse these. **Vanna, volga, charm, the CRR American tree, and the delta-hedging simulator are NOT yet in the file** — implement them by extending it; never assume they exist.
