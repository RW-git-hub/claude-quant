...
A single train/test path gives one estimate of OOS performance — fragile under non-stationarity. CPCV splits the data into `N` contiguous groups, then forms all `C(N, k)` combinations that leave out `k` groups for testing, with purge+embargo applied around each test group. This yields *many* backtest paths (`C(N-1, k-1)` distinct paths) from the same data, producing a **distribution** of OOS Sharpes instead of a point estimate. That distribution is the raw material for the Probability of Backtest Overfitting (PBO) — see `references/stats-risk.md`. CPCV is the recommended evaluation engine for ML alpha; `templates/validation.py` provides it as `CombinatorialPurgedKFold`.
...
| **Backtest overfitting from many tries** | Researcher tests many features/models/labels on the same data | Best result indistinguishable from best-of-N noise | CPCV path distribution; report DSR and PBO (`references/stats-risk.md`); pre-register the test |
...
- `references/stats-risk.md` — Deflated Sharpe Ratio (DSR) and Probability of Backtest Overfitting (PBO) for multiple-testing correction.
