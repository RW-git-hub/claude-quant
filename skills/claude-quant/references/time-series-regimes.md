...
`lam` (decay) sets memory: the weight on the lag-`k` past squared return is `(1-lam)*lam^k`, so the center-of-mass (mean lag) ≈ `lam/(1-lam)` periods (0.94 → ~15.7 ≈ 16 days), while the *half-life* (where `lam^h = 1/2`) is `ln(0.5)/ln(lam)` ≈ 11 days at lam=0.94. Higher `lam` = smoother, slower to react. Note the index: `sigma^2_t` (forecast for day `t`) depends on `r^2_{t-1}` and `sigma^2_{t-1}` — strictly past info, so it is correctly lagged *by construction* when used to size a position entered at the close of `t-1` for day `t`.
...
Use `arch` (`from arch import arch_model; arch_model(100*rets, vol="GARCH", p=1, q=1).fit()`). Scale returns to ~percent units for numerical stability, and divide forecast variances back by `100**2` before use. For a genuine OOS forecast, refit on a rolling window and use `forecast(horizon=...)`, not in-sample conditional volatility. (`templates/regime.py` ships a dependency-free variance-targeting `garch11_fit`/`garch11_filter` if you want auditable internals.)

### GJR-GARCH / EGARCH — the leverage effect (asymmetric vol)
The single most useful generalization on equities/indices: **negative returns raise next-period vol more than equal-magnitude positive returns** (the "leverage effect"). GJR-GARCH(1,1) adds one parameter:

```
sigma^2_t = omega + (alpha + gamma * 1[r_{t-1} < 0]) * r^2_{t-1} + beta * sigma^2_{t-1}
```

- `gamma > 0` is the asymmetry; `gamma = 0` collapses exactly to GARCH(1,1). On equity indices `gamma` is reliably positive and often comparable to or larger than `alpha`.
- **Stationarity / unconditional variance** (under a symmetric innovation, so `P(r<0)=1/2`, the expected ARCH multiplier is `alpha + gamma/2`): effective persistence `= alpha + gamma/2 + beta < 1`, and `uncond_var = omega / (1 - alpha - gamma/2 - beta)`. Variance-target on this denominator. Also require `alpha + gamma >= 0` so the negative-shock multiplier stays non-negative.
- **EGARCH** (Nelson) models `log sigma^2_t` with an asymmetric news-impact term; it guarantees positivity without sign constraints and captures asymmetry via the sign of the standardized shock. Either GJR or EGARCH (often with `dist="t"` for fat tails) **usually beats plain Gaussian GARCH on equities/indices.** Pick GJR for interpretability of the threshold, EGARCH if you need unconstrained positivity.
- Compare models on the **same** likelihood basis (e.g. Gaussian NLL, or an information criterion) so the extra parameter is paying its way; `gamma ≈ 0` or no NLL improvement ⇒ don't carry the asymmetry term. `gamma < 0` (up-moves raise vol more) is unusual for equities — sanity-check sign conventions, though it can legitimately occur for some commodities/FX.

Use `arch` (`arch_model(100*rets, vol="GARCH", p=1, o=1, q=1, dist="t")` gives GJR; `vol="EGARCH"` gives EGARCH). `templates/regime.py` ships a dependency-free `gjr_garch11_fit`/`gjr_garch11_filter` (variance-targeted, same Gaussian-NLL machinery as the symmetric fit so the two are directly comparable) for auditable internals and unit-testing the leverage recovery.
...
- `RV` is a far more accurate proxy for latent vol than `r_t^2`; **HAR-RV** (Corsi 2009: regress `RV_t` on `RV_{t-1}`, the trailing weekly avg, and the trailing monthly avg) is a strong, simple daily-vol forecaster and a good baseline to beat GARCH. When forecasting, all RHS terms must end at `t-1` (no contemporaneous `RV_t` on the right). `templates/regime.py` ships a runnable `har_rv` (strictly-trailing OLS design, optional log-RV fit, with a no-look-ahead assertion baked in) plus a `realized_variance` helper (with an optional overnight term).
  - HAR design (all RHS strictly through `t-1`): `RV^{(d)}=RV_{t-1}`, `RV^{(w)}=mean(RV_{t-5..t-1})`, `RV^{(m)}=mean(RV_{t-22..t-1})`; fit `RV_t = c + b_d RV^{(d)} + b_w RV^{(w)} + b_m RV^{(m)} + eps` by OLS. For an `h`-day forecast, set the LHS target to `mean(RV_{t..t+h-1})` (the *thing predicted* — never a regressor) and leave the RHS unchanged. Fitting in **log-RV** tames RV's right-skew/heteroskedasticity and often forecasts better (exponentiate back; a small retransformation bias remains — correct with a smearing factor if you need an unbiased level). HAR's three lagged aggregates approximate long-memory in RV with only ~4 parameters and no fractional differencing.
...
```
Even the forward/filtered posterior uses the *fitted parameters*, which were learned from a window; refit only on a walk-forward expanding/rolling window (use `templates/validation.py`), never once on the full history. A 2-state Gaussian HMM on equity returns *often* separates a low-vol/positive-drift state from a high-vol/negative-drift state — useful as a crisis switch — but this is not guaranteed every refit; validate the states and beware label switching (below). (`templates/regime.py::hmm_gaussian_2state` is a dependency-free Baum-Welch + Viterbi fit with canonical low/high-variance state ordering.)
...
```
`mu_0` is the in-control reference mean and must be estimated from past/in-control data only. `k` (slack/reference value) ≈ half the smallest shift worth detecting; `h` trades off detection delay vs false-alarm rate. Cheap, online, one-sided or two-sided. Commonly run on *vol* or a *spread mean* rather than raw price. (`templates/regime.py::cusum_changepoints` re-estimates the in-control mean per segment so it does not false-alarm at `t=0`.)
...
- **Differencing:** `d` = number of differences to reach stationarity. Don't over-difference (introduces spurious negative MA structure / inflates variance). For fractional persistence, ARFIMA / fractional differencing preserves more memory while reaching stationarity (useful for long-memory series like RV — though HAR-RV's three lagged aggregates approximate that long memory with far fewer parameters; see §1).
...
```
- `sigma_hat_{t-1}` is the lagged vol forecast (EWMA/GARCH/GJR/HAR-RV) — **never** the contemporaneous one (Section 1 CRITICAL). See `templates/backtest_skeleton.py::vol_target_sizer` and `templates/regime.py::vol_target_scale`.
...
| Pitfall | Detect | Fix |
|---|---|---|
| **Look-ahead in vol/regime estimate** (estimate at `t` includes `r_t`, then sizes day `t`) | Re-run all metrics with the vol/regime input `.shift(1)`; a large Sharpe/drawdown change ⇒ you were leaking. `ewm(...).std()` is indexed with the current bar. | Fix the convention `pos_t = f(estimate_{t-1})`; unit-test the sizing fn never reads a return ≥ the bar it sizes (see `regime.py::har_rv`'s internal assertion + lag test). |
| **Look-ahead in HAR-RV** (contemporaneous `RV_t` on the RHS, or the `h`-step target leaking into regressors) | Forecast "predicts" RV implausibly well; corrupting `RV_t` and later changes the forecast at `t`. | Build every RHS aggregate to end at `t-1`; keep the multi-step target (`mean RV_{t..t+h-1}`) strictly on the LHS; assert the forecast at `t` is invariant to corrupting `RV_{>=t}`. |
...
| **Ignoring the leverage effect** (symmetric GARCH on equities) | News-impact curve is asymmetric; symmetric GARCH under-predicts vol after sell-offs; GJR/EGARCH fit shows `gamma > 0` with a materially lower NLL/AIC. | Fit **GJR-GARCH** or **EGARCH** (variance-target the `1 - alpha - gamma/2 - beta` denominator); compare to symmetric GARCH on the SAME likelihood/IC; carry `gamma` only if it pays. (`regime.py::gjr_garch11_fit`.) |
...
| **Microstructure noise / overnight gap in RV** (RV biased high at fine sampling, or biased low by missing the overnight jump) | RV explodes as sampling → finest; RV-vol systematically below daily-return GARCH vol for a gappy single name. | Sample ~5-min or use realized-kernel/TSRV de-biasing; **add the overnight squared close-to-open term** (`realized_variance(..., overnight_returns=...)`); deseasonalize 24h markets. |
...
See also: `references/pitfalls.md` (general look-ahead/overfit), `references/stats-risk.md` (Sharpe, bootstrap, Newey-West), `references/factor-research.md` (IC, cross-sectional), `references/stat-arb.md` and `templates/pairs_trading.py` (Kalman hedge ratios), `references/microstructure.md` (crisis-liquidity costs), and `templates/validation.py` (`PurgedKFold`/`CombinatorialPurgedKFold`) + `templates/backtest_skeleton.py` (`vol_target_sizer`) + `templates/regime.py` (EWMA / GARCH / GJR-GARCH / HAR-RV / HMM / Kalman / CUSUM / vol-target) for runnable implementations.
