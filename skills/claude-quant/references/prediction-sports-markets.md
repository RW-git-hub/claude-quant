...
> **Template status (read first).** `templates/betting_markets.py` ships the core, self-tested utilities for this reference: odds conversions, devigging (multiplicative / power / Shin), expected value, Kelly sizing, Brier score, log loss, and closing-line value. Note that `templates/metrics.py` covers return/Sharpe/Sortino/drawdown/VaR-style performance metrics only (no calibration utilities) — calibration lives in `betting_markets.py`. Reliability diagrams, Platt/isotonic recalibration, and the bankroll-simulation snippets below are illustrative pseudocode to adapt. For time-series CV reuse `templates/validation.py` (`PurgedKFold` / `CombinatorialPurgedKFold`, purge + embargo).
...
- **Brier score:** `BS = mean((p − y)²)`. Lower is better; decomposes (Murphy) into reliability − resolution + uncertainty.
- **Log loss:** `−mean(y·ln p + (1−y)·ln(1−p))`. Punishes confident wrong calls harshly; clip `p` to `[ε, 1−ε]` to avoid `ln(0)`.
- **Reliability diagram:** bin predictions by `p`, plot mean predicted vs observed frequency per bin; the 45° line is perfect calibration. Realized below predicted = overconfident (and reveals favorite–longshot bias in market prices).

**Recalibration:**
- **Platt scaling:** fit a logistic `σ(a·logit(p) + c)` mapping raw scores to calibrated probs. Parametric, good for small data.
- **Isotonic regression:** fit a non-decreasing step function from predicted to observed; flexible, needs more data, can overfit small samples.
- Fit recalibration on a **separate** fold from training and from evaluation — recalibrating on the test set leaks.

> Implementation note: these calibration utilities are **not** in any current template (`templates/metrics.py` is performance metrics only — Sharpe/Sortino/drawdown/VaR). Use `sklearn.metrics.brier_score_loss` / `log_loss`, `sklearn.calibration.calibration_curve`, and `sklearn.calibration.CalibratedClassifierCV` (or `sklearn.isotonic.IsotonicRegression`), or implement them yourself.
...
| Miscalibrated model probabilities | Reliability diagram bows off the diagonal; Brier/log loss poor | Recalibrate (Platt/isotonic) on a held-out fold before feeding `p` into edge/Kelly (§15) |
...
| Importing nonexistent template code | `from templates.betting_markets import ...` fails; calibration helpers assumed in `metrics.py` | This domain has **no dedicated template** — implement devig/Kelly/CLV yourself; use sklearn for calibration; reuse `validation.py` for purge/embargo only |

---

**Templates that actually exist and apply here:** `templates/validation.py` (`PurgedKFold` / `CombinatorialPurgedKFold` for purge + embargo, §16) and `templates/metrics.py` (performance metrics — Sharpe, Sortino, drawdown, VaR, deflated/probabilistic Sharpe; **not** calibration). `templates/betting_markets.py` ships odds conversions, devig (multiplicative/Shin/power), expected value, Kelly & fractional Kelly, Brier/log-loss, and CLV; the reliability-diagram and Platt/isotonic recalibration snippets above are illustrative (scikit-learn supplies isotonic/Platt). See also `references/pitfalls.md` and `references/stats-risk.md` for the general look-ahead, survivorship, and overfitting framing that applies here verbatim.