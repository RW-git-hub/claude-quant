...
2. **Pick the scheme.** Walk-forward = chronological, time-ordered `PurgedKFold` splits that mimic live retraining (do NOT shuffle). Want many backtest paths / a Sharpe *distribution* → `CombinatorialPurgedKFold`; feed the resulting path distribution to the **overfitting-detective** agent for the PBO / deflated-Sharpe *verdict* (this skill produces the OOS paths; it does not pronounce the overfit verdict). There is no separate WalkForward class — walk-forward is just `PurgedKFold` consumed in time order.
