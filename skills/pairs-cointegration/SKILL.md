...
description: 'Use when asked to "build a pairs trade", "find a cointegrated pair", "trade a mean-reverting spread", "Engle-Granger / Johansen test", "estimate the hedge ratio / half-life", "z-score spread strategy", or "stat-arb pair" — the quick end-to-end playbook for ONE cointegration pair (the broad claude-quant skill is the full-lifecycle router).'
allowed-tools:
  - Read
  - Write
  - Edit
...
5. **Rolling hedge ratio (no look-ahead)**: use `kalman_hedge_ratio(y, x)` (point-in-time), or apply `hedge_ratio` over a trailing window — NEVER full-sample beta to trade OOS. Build the spread with `spread(y, x, beta, intercept)`.
